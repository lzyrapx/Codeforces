...
bool check_segment(const Point &p, const Point &a, const Point &b)//a:是下面的点，b:是上面的点，斜边的公共点 
{
    if (online(p, a, b))
	{
    ...
    for (int first = 0; first < 3; ++ first)
	{
        for (int second = first + 1; second < 3; ++ second)
		{
            ...
            if (found==true)
			{
                ...
                if (check_angle(segments[first][0] - common, segments[second][0] - common))//必须存在夹角 
				{
